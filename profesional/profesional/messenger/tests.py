...
class ThreadTestCase(TestCase):
    ...
    def test_add_messages_to_thread(self):
        self.thread.users.add(self.user1, self.user2)
        message1 = Message.objects.create(user=self.user1, content='hola Irving')
        message2 = Message.objects.create(user=self.user2, content='hola Carlos')
    ...
    def test_add_message_from_user_not_in_thread(self):
        ...
        message1 = Message.objects.create(user=self.user1, content='hola Irving')
        message2 = Message.objects.create(user=self.user2, content='hola Carlos')
        message3 = Message.objects.create(user=self.user3, content='Hola Soy un Hacker')
        self.thread.messages.add(message1, message2, message3)
        self.assertEqual(len(self.thread.messages.all()), 2)

    def test_find_threat_with_custom_manager(self):
        self.thread.users.add(self.user1, self.user2)
        thread = Thread.objects.find(self.user1, self.user2)
        self.assertEqual(self.thread, thread)
        
    def test_find_or_create_threat_with_custom_manager(self):
        self.thread.users.add(self.user1, self.user2)
